  ...
  return (
    <View
      ...
      <LineChart
        // area configuration
        areaChart
        startFillColor={color.gradientFromSwipeable}
        ...
        initialSpacing={10}
        color={color.gradientFromSwipeable}
        thickness={2}
        isAnimated={true}
...
export const CryptoChart = ({ objData, to, maxValue, minValue }) => {
  //   object to arr
  const arrData = Object.values(objData || {}).map((key) => {
    return {
      date: moment.unix(key.time).format('DD-MM-YYYY'),
      value: key.close || 0,
      defaultValue: key.close || 0,
    };
  });
